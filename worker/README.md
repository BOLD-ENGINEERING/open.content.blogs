...
Linux with glibc is required for atomic directory exchange (`renameat2`).
...
character outside `[a-z0-9-]` becomes `-`. Browsers resolve these names locally.
...
The worker supplies `CONTENT_DIR`, `BLOG_CONFIG`, `BUILD_OUT_DIR`, and `SITE_URL`
to Astro. Their standalone defaults and relative-path behavior are in the root
AGENTS.md Build contract. Builds use a minimal PATH and private HOME. Concurrent
builds serialize template access with a lock under `/tmp/ocb-template-locks`, keyed
by the resolved template path; different BUILD_ROOT values still share the lock.
The template remains Git-clean; ignored Astro cache files may be regenerated.
Stylesheets are inlined to avoid Astro 7's cross-filesystem CSS rename limitation.
...
exceeding any fails the entire fetch and removes its UUID directory. One Git log
...
retain logs; fetch failures return their error and manifest with no build directory.
...
Tests never use Cloudflare. The thirteen pipeline cases include S4b, plus four
visual combinations (two sizes, two themes). The visual cases produce 36 PNGs in
`docs/qa/phase-2/screenshots/` and per-page transfer/accessibility metrics in the
run's root. Axe 4.13.0 is vendored with its license and injected locally.
...
keyboard focus, zero serious/critical axe violations and transfer below 200 KiB.
